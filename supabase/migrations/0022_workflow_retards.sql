-- Gestion des délais (§7 du prompt workflow V2) : détecte les instances de
-- workflow en retard (etape_courante_depuis + workflow_etapes.delai_jours
-- dépassé) et notifie (relance) les acteurs concernés + (escalade) le
-- supérieur hiérarchique de l'entité porteuse. Appelée périodiquement par la
-- Edge Function `detecter-retards-workflow` (pas de dépendance à pg_cron: la
-- planification se fait via le Cron Jobs de la Edge Function elle-même,
-- configuré côté tableau de bord Supabase).
...
begin
  ...
  loop
    ...
    -- Une seule notification de retard par objet et par jour, pour éviter le
    -- spam à chaque exécution planifiée (ex. toutes les heures).
    select exists (
    ...
    -- Relance: mêmes acteurs que ceux qui seraient notifiés pour agir sur
    -- cette étape (workflow_transition_roles des transitions sortantes).
    for v_acteur in
      select distinct wtr.*
    ...
    -- Escalade: notifie en plus le supérieur hiérarchique de l'entité
    -- porteuse (responsable de l'entité parente), s'il existe.
